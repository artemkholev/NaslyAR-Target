...
export const Quations = () => {
  ...
  const quationsContent = quationsArray.map((item) => (
    <li
      key={item.id}
      className={`${
        item.open ? "bg-[var(--white-bg)]" : "bg-[var(--component-bg)]"
      } rounded-[30px] py-4 max-md:h-auto px-8 w-full mb-4 shadow-lg`}>
      <div
        className='w-full flex flex-row justify-between gap-5 items-center cursor-pointer'
        onClick={() => toggleOpen(item)}>
        <p className='typography__title font-bold flex-1 break-words text-left'>{item.title}</p>
        <OpenCloseButton isOpen={item.open} onToggle={() => {}} />
      </div>
      {item.open && <div className='typography__text mt-6 whitespace-pre-line'>{item.discription}</div>}
    </li>
  ));

  return (
    <article id='quations' className='page__box'>
      <h1 className='typography__title font-bold text-center'>
